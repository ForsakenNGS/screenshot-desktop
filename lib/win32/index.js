...
function windowsCrop(region) {
  if (typeof region == "undefined") {
    return "";
  }
  return " "+(screen.offsetX+" "+screen.offsetY+" "+region.w+" "+region.h).trim();
}
...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const crop = (displayChoice !== '' ? windowsCrop(options.region) : '');
    console.error(__dirname);
    /*exec('"' + path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice + crop, {
