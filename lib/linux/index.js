...
function linuxCrop(region, screen) {
  if (typeof region == "undefined") {
    return null;
  }
  return region.w+"x"+region.h+"+"+(screen.offsetX+region.x)+"+"+(screen.offsetY+region.y);
}
...
function linuxSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    listDisplays().then((screens) => {
      ...
      const crop = linuxCrop(options.region, screen) || screen.crop;

      exec(
        `import -silent -window root -crop ${crop} -screen ${filetype}:"${filename}" `,
        execOptions,
        (err, stdout) => {
          if (err) {
            return reject(err)
